...
class ReplayBuffer:
    ...
    weights = collections.deque(maxlen=5000)
    probabilities = collections.deque(maxlen=5000)

    def buffer_append(self,transition, weight, probability):
        if len(self.buffer) == 10000:
            self.buffer.popleft()
            self.weights.popleft()
            self.probabilities.popleft()
...
if __name__ == "__main__":
    ...
    for epoch in range(200):
        ...
        for step_num in range(episode_length):
            ...
            q_values_state = dqn.q_network.forward(torch.unsqueeze(state_tensor, 0))
            #print(q_values_state)
            best_action = torch.max(q_values_state,1)[1][0].numpy()
            #print(best_action)
            # Step the agent once, and get the transition tuple for this step
            transition = agent.step(best_action)
            state = transition[3]

            if len(my_buffer.weights) == 0:
                weight = 0
                probability = 1
            else:
                weight = max(my_buffer.weights)

            # Calculate new loss function for this step
            #loss = dqn.train_q_network(transition)
            my_buffer.buffer_append(transition, weight, probability)

            if len(my_buffer.buffer) >= 1000:
                minibatch = my_buffer.minibatch(100)
    ...
    plt.xlabel('Episodes')
    plt.title('DQN with Target Network - Average Loss per Episode')
    plt.savefig('loss_target_network')
