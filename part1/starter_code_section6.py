...
class DQN:
    ...
    def _calculate_loss(self, minibatch):
        ...
        minibatch_inputs = minibatch[0]
        minibatch_outputs = minibatch[2]
        minibatch_input_tensor = torch.tensor(minibatch_inputs)
        minibatch_labels_tensor = torch.tensor(minibatch_outputs)
        ...
        network_prediction = self.q_network.forward(minibatch_input_tensor)
        #print(network_prediction)
        #print(minibatch[1])
        network_prediction = torch.gather(network_prediction,1,torch.tensor(minibatch[1]).unsqueeze(-1)).squeeze(-1)
        #print(network_prediction)
        # Compute the loss based on the label's batch
        loss = torch.nn.MSELoss()(minibatch_labels_tensor, network_prediction)
        #print(loss)
        return loss
...
if __name__ == "__main__":
    ...
    for epoch in range(100):
        ...
        for step_num in range(episode_length):
            ...
            my_buffer.buffer_append(transition)
            #print(my_buffer.buffer)
            if len(my_buffer.buffer) > 100:
    ...
    plt.ylabel('Loss')
    plt.xlabel('Episodes')
    plt.title('Experience Replay Buffer (batch of 100) - Average Loss per Episode')
    plt.savefig('loss_section6')
