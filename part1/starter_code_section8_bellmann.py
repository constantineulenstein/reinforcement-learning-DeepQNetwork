...
class DQN:
    ...
    def _calculate_loss(self, minibatch):
        ...
        nextstate_q_values = self.q_network.forward(minibatch_nextstates_tensor)

        nextstate_maxq_values = torch.max(nextstate_q_values,1)[0]
        ...
        target_tensor = torch.add(minibatch_rewards_tensor, nextstate_maxq_values, alpha = 0.9)
        #print(target_tensor)
...
if __name__ == "__main__":
    ...
    plt.xlabel('Episodes')
    plt.title('DQN without Target Network - Average Loss per Episode')
    plt.savefig('loss_belmann')
